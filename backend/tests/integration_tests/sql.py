...
from bracket.models.db.team import TeamBracket, TeamInsertable
...
@asynccontextmanager
async def inserted_team(team: TeamInsertable) -> AsyncIterator[TeamBracket]:
    async with inserted_generic(team, teams, TeamBracket) as row_inserted:
        yield cast("TeamBracket", row_inserted)
...
async def inserted_stage(stage: StageInsertable) -> AsyncIterator[Stage]:
    async with inserted_generic(stage, stages, Stage) as row_inserted:
        try:
            ...
            stage_item_ids = select(stage_items.c.id).where(
                stage_items.c.stage_id == row_inserted.id
            )
            round_ids = select(rounds.c.id).where(rounds.c.stage_item_id.in_(stage_item_ids))
            await database.execute(matches.delete().where(matches.c.round_id.in_(round_ids)))
            await database.execute(
                stage_item_inputs.delete().where(
                    stage_item_inputs.c.stage_item_id.in_(stage_item_ids)
                )
            )
            await database.execute(rounds.delete().where(rounds.c.stage_item_id.in_(stage_item_ids)))
            await database.execute(stage_items.delete().where(stage_items.c.stage_id == row_inserted.id))
...
async def inserted_stage_item(stage_item: StageItemInsertable) -> AsyncIterator[StageItem]:
    async with inserted_generic(stage_item, stage_items, StageItem) as row_inserted:
        try:
            ...
            round_ids = select(rounds.c.id).where(rounds.c.stage_item_id == row_inserted.id)
            await database.execute(matches.delete().where(matches.c.round_id.in_(round_ids)))
            await database.execute(
                stage_item_inputs.delete().where(
                    stage_item_inputs.c.stage_item_id == row_inserted.id
                )
            )
            await database.execute(rounds.delete().where(rounds.c.stage_item_id == row_inserted.id))
