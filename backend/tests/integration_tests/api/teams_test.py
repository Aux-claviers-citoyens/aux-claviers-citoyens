...
from bracket.models.db.team import TeamBracket
...
async def test_create_teams(
    ...
    body = {"names": "TeamBracket -1,Player 42,Player 43\nTeam -2,", "active": True}
...
async def test_update_team(
    ...
    body = {"name": "Some new name", "active": True, "player_ids": [], "logo_path": "existing.png"}
    async with inserted_team(
        DUMMY_TEAM1.model_copy(update={"tournament_id": auth_context.tournament.id})
    ) as team_inserted:
        response = await send_tournament_request(
        ...
        updated_team = await fetch_one_parsed_certain(
            database, TeamBracket, query=teams.select().where(teams.c.id == team_inserted.id)
        )
        assert updated_team.name == body["name"]
        assert updated_team.logo_path == body["logo_path"]
        assert response["data"]["name"] == body["name"]
        assert response["data"]["logo_path"] == body["logo_path"]
