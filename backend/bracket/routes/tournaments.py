...
async def get_tournament(
    ...
    user: UserPublic | None = Depends(user_authenticated_optional),
) -> TournamentResponse:
...
async def get_tournaments(
    ...
    user_club_ids = await get_which_clubs_has_user_access_to(1)

    if isinstance(user, UserPublic):
        return TournamentsResponse(
            data=await sql_get_tournaments(user_club_ids)
        )

    return TournamentsResponse(
        data=await sql_get_tournaments(user_club_ids, None, "OPEN")
    )
...
async def create_tournament(
    tournament_to_insert: TournamentCreateBody, user: UserPublic = Depends(user_authenticated)
) -> TournamentResponse:
    # If club_id is omitted, infer it when possible
    # Déterminer le club_id (non documenté dans le body)
    user_club_ids = await get_which_clubs_has_user_access_to(user.id)
    if len(user_club_ids) == 1:
        club_id = next(iter(user_club_ids))
    else:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="club_id is required when you have access to multiple clubs",
            headers={"WWW-Authenticate": "Bearer"},
        )

    # Construire le TournamentBody interne avec club_id
    tournament_to_insert = TournamentBody(**tournament_to_insert.model_dump(), club_id=club_id)  # type: ignore[arg-type]
