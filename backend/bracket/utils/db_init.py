...
from bracket.models.db.club import ClubInsertable, ClubCreateBody
...
from bracket.sql.clubs import create_club, sql_give_user_access_to_club
...
async def init_db_when_empty() -> UserId | None:
    ...
    if config.admin_email and config.admin_password:
        if (table_count <= 1 and environment != Environment.CI) or (
            ...
            # Create a default club and assign the admin as owner
            logger.warning("Creating default club and assigning admin as owner...")
            await create_club(ClubCreateBody(name=config.default_club_name), admin_user_id)
...
async def ensure_admin_default_club_link() -> None:
    """
    Ensure the admin user (from config) is linked to the default club.
    - If the club doesn't exist, create it and link via create_club (which links OWNER).
    - If the club exists but link is missing, add it.
    ...
    club_row = await database.fetch_one(
        "SELECT id FROM clubs WHERE name = :name",
        {"name": config.default_club_name},
    )

    if club_row is None:
        await create_club(ClubCreateBody(name=config.default_club_name), admin.id)
        return

    club_id = ClubId(club_row["id"])  # type: ignore[index]
    ...
    link_row = await database.fetch_one(
        ...
        """,
        {"user_id": admin.id, "club_id": club_id},
    )
    if link_row is None:
        await sql_give_user_access_to_club(admin.id, club_id)
...
async def sql_create_dev_db() -> UserId:
    ...
    team_id_5 = await insert_dummy(
        DUMMY_TEAM4, TeamId, {"name": "TeamBracket 5", "tournament_id": tournament_id_1}
    )
    team_id_6 = await insert_dummy(
        DUMMY_TEAM4, TeamId, {"name": "TeamBracket 6", "tournament_id": tournament_id_1}
    )
    team_id_7 = await insert_dummy(
        DUMMY_TEAM4, TeamId, {"name": "TeamBracket 7", "tournament_id": tournament_id_1}
    )
    team_id_8 = await insert_dummy(
        DUMMY_TEAM4, TeamId, {"name": "TeamBracket 8", "tournament_id": tournament_id_1}
    )
