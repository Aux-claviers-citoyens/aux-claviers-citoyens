...
from bracket.models.db.team import FullTeamWithPlayers, TeamBracket
...
async def get_teams_by_id(team_ids: set[TeamId], tournament_id: TournamentId) -> list[TeamBracket]:
    if len(team_ids) < 1:
        return []
    ...
    return [TeamBracket.model_validate(team) for team in result]


async def get_team_by_id(team_id: TeamId, tournament_id: TournamentId) -> TeamBracket | None:
