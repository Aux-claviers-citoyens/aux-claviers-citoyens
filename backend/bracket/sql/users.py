...
async def create_user(user: UserInsertable) -> User:
    ...
    created_user = User.model_validate(dict(assert_some(result)._mapping))

    # Give new users access to default club (id=1) as COLLABORATOR if it exists
    club_exists = await database.fetch_val("SELECT 1 FROM clubs WHERE id = 1")
    if club_exists is not None:
        link_exists = await database.fetch_val(
            """
            SELECT 1 FROM users_x_clubs WHERE user_id = :user_id AND club_id = 1
            """,
